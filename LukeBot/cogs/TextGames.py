...
class TextGames(commands.Cog):
    ...
    async def type_race(self, ctx):
        ...
        Database.cmd_to_db(
            ctx.command.name,
            "DM" if not ctx.guild else str(ctx.guild.id),
            str(ctx.author.id),
        )

        await ctx.send(
            f"Please type 'race' to join/leave race. {ctx.author.mention} may begin the race by typing 'start' or cancel by typing 'cancel'"
        )
        ...
        while True:
            ...
            if user_message_content == "race":  # user types race
                if (
                    user_message.author in racing_users and not ctx.author
                ):  # if user is not author and has already entered race, he now exits race.
                ...
                ):  # if user is not author and types race
                    await ctx.send(f"User {user_message.author.mention} added to race.")
                    racing_users.add(user_message.author)

            if (
                user_message_content == "cancel"
            ):  # if typerace initiator types *cancel, race is canceled. return to end function
                await ctx.send("Race Cancelled.")
                return

            if (
                user_message_content == "start" and user_message.author == ctx.author
            ):  # typerace initiator types *start, race begins
                break

        # grab typing prompt
        with open(f"{PROJECT_DIR}/assets/typing_prompts.txt", "r") as typing_file:
            typing_prompt = typing_prompt = random.choice(
                typing_file.readlines()
            ).rstrip("\n")
        ...
        while time.time() < init_time + 45 and len(racing_users) != 0:
            ...
            try:
                user_message = await self.bot.wait_for(
                    "message",
                    check=lambda mess: mess.author in racing_users,
                    timeout=0.5,
                )
            ...
            if user_message_content == "end":  # end race early
                await ctx.send(
                    f"{user_message.author.mention} has ended the race early."
                )
                break
            ...
            if user_message_content == typing_prompt:  # user types prompt correctly
                ...
                user_data[user_message.author] = (len(typing_prompt) / 5) / (
            ...
            if (
                init_time + 45 - stopwatch < 11
                and init_time + 45 - stopwatch > 9
                and not one_warning
            ):  # 10 second warning
                one_warning = True
    ...
    async def wordle(self, ctx):
        ...
        def create_grid(
            guesses: list[str] = None, word: str = ""
        ) -> discord.File:  # creates and fills grid based on list of user guesses
            if guesses is None:
                guesses = []

            grid = Image.new(
                "RGB", (500, 600), color=(207, 207, 207)
                ...
                if guesses[i]:  # if not empty
                    ...
                    for j, char in enumerate(guesses[i]):
                        if char == word[j]:  # correct char = green
                            ...
                            grid.paste(
                                Image.open(
                                    f"{PROJECT_DIR}/assets/wordle/GreenSquare.png"
                                ),
                                (j * 100, i * 100),
                            )
                            text_plane.text(
                    ...
                    for j, char in enumerate(guesses[i]):
                        if (
                            char in temp_word and char != word[j]
                        ):  # incorrect but in word = yellow
                            temp_word = temp_word.replace(char, "", 1)
                            grid.paste(
                                Image.open(
                                    f"{PROJECT_DIR}/assets/wordle/YellowSquare.png"
                                ),
                                (j * 100, i * 100),
                            )
                            text_plane.text(
                        ...
                        # if green/yellow exhausted of that char, should be gray
                            ...
                            grid.paste(
                                Image.open(
                                    f"{PROJECT_DIR}/assets/wordle/ColorAbsent.png"
                                ),
                                (j * 100, i * 100),
                            )
                            text_plane.text(
            ...
            grid.save(f"{PROJECT_DIR}/assets/wordle/grid.png")
            grid.close()
            return discord.File(f"{PROJECT_DIR}/assets/wordle/grid.png")

        # Set up initial embed below
        wordle_embed = discord.Embed(
            title="WORDLE", description="Type any five-letter word to guess!"
        )
        grid_file = create_grid()  # create empty grid to start
        wordle_embed.set_image(url="attachment://grid.png")

        update = await ctx.send(
            embed=wordle_embed, file=grid_file
        )  # store embed message to update embed later
        ...
        rand_words = (
            []
        )  # will get answer word as well as 6 words in case user does not enter word within time limit
        ...
        for i in range(1, 7):  # loop through six guesses
            ...
            try:
                user_guess = await self.bot.wait_for(
                    "message",
                    timeout=120.0,
                    check=lambda mess: mess.author == ctx.author
                    and len(mess.content) == 5,
                )
            except asyncio.TimeoutError:
                await ctx.send(
                    f"Time limit for guess exceeded! Random word {rand_words[0]} guessed."
                )
                flag_time = True
            ...
            if flag_time == False:  # add user guess if user guesses in time
                ...
                guess_list.append(
                    rand_words[0]
                )  # if time limit exceeded, add from randWord
                rand_words.remove(rand_words[0])

            grid_file = create_grid(guess_list, ans_word)
            await update.edit(
                embed=wordle_embed, attachments=[grid_file]
            )  # update embed (grid image has changed)
            if guess == ans_word:  # WIN CASE
                await ctx.send(
                    f"Congratulations! {ctx.author.mention} won wordle! The word was: {ans_word}. Guesses taken: {len(guess_list)}"
                )
                flag_guess = True
                break
        ...
        if flag_guess == False:
            await ctx.send(
                f"{ctx.author.mention} has failed wordle. The word was: {ans_word}"
            )

        os.remove(
            f"{PROJECT_DIR}/assets/wordle/grid.png"
        )  # delete generated grid image
    ...
    async def picdle(self, ctx, difficulty: str = None):
        ...
        if not difficulty:
            await ctx.send(
                "Please try again and enter a difficulty. see *help for options."
            )
            return

        # Image from URL into bytes to send in discord embed (tested here using a different method, not downloading images)
        def url_to_imgfile(
            response: httpx.Response, embed: discord.Embed, blur: int = None
        ) -> discord.File:
        ...
        while True:
            ...
            if response.status_code == 200:
                animal_embed = discord.Embed(
                    title="Cat or Dog?",
                    description="To guess, type 'cat' or 'dog'. 'end' will end the game.",
                )
            ...
            try:
                user_guess = await self.bot.wait_for(
                    ...
                    and (
                        mess.content == "cat"
                        or mess.content.lower() == "dog"
                        or mess.content.lower() == "end"
                    ),
                )
            except (
                asyncio.TimeoutError
            ):  # if time out, show unblurred image and end game
                await ctx.send(
                    f"Time limit for guess exceeded! Answer was: {animal_options[coinflip]}"
                )
                await update.edit(embed=animal_embed, attachments=[file_image])
                return

            await update.edit(
                embed=animal_embed, attachments=[file_image]
            )  # update discord embed with unblurred image
            ...
            # user wrong guess
                ...
                await ctx.send(
                    f"Wrong! Answer was: {animal_options[coinflip]}. Game Over!"
                )
                return

    @commands.hybrid_command(
        name="ttt", description="challenge another user to tic-tac-toe"
    )
    async def ttt(self, ctx, user: discord.Member = None):
        ...
        def add_turn(
            first_start: bool, place: int = None, letter: chr = None
        ) -> discord.File:
            if first_start == True:
                grid = Image.open(
                    f"{PROJECT_DIR}/assets/tictactoe/grid.png"
                )  # create image background
            else:
                grid = Image.open(f"{PROJECT_DIR}/assets/tictactoe/gridNEW.png")
            match place:
                case 1:
                    grid.paste(
                        Image.open(f"{PROJECT_DIR}/assets/tictactoe/{letter}.png"),
                        (0, 0),
                    )
                case 2:
                    grid.paste(
                ...
                case _:
                    pass
            grid.save(f"{PROJECT_DIR}/assets/tictactoe/gridNEW.png")
            grid.close()
            return discord.File(f"{PROJECT_DIR}/assets/tictactoe/gridNEW.png")

        # discord user accept/deny challenge
        await ctx.send(
            f"{user.mention}: You have 10 seconds to accept or deny {ctx.author.mention}'s TIC-TAC-TOE challenge. 'accept' or 'deny'."
        )
        try:
            user_ans = await self.bot.wait_for(
                "message",
                check=lambda mess: mess.author == user
                and (
                    mess.content.lower() == "accept" or mess.content.lower() == "deny"
                ),
                timeout=10.0,
            )
        except asyncio.TimeoutError:
        ...
        ttt_embed = discord.Embed(
            title="TIC-TAC-TOE", description="Type desired grid number for your turn."
        )
        ...
        for i in range(9):
            ...
            try:
                if i % 2 == 0:
                    msg = await self.bot.wait_for(
                        ...
                        and (
                            int(mess.content) < 10
                            and int(mess.content) > 0
                            and user_placed[int(mess.content)] is None
                        ),
                        timeout=15.0,
                    )
            ...
            if i > 3:  # no need to check unless 3 x's minimum have been placed
                for win in win_combinations:
                    if (
                        user_placed[win[0]] == "x"
                        and user_placed[win[1]] == "x"
                        and user_placed[win[2]] == "x"
                    ):
                        await ctx.send(
                            f"TIC-TAC-TOE!: User {ctx.author.mention} has won!"
                        )
                        flag = True
                        break
                    elif (
                        user_placed[win[0]] == "o"
                        and user_placed[win[1]] == "o"
                        and user_placed[win[2]] == "o"
                    ):
                        await ctx.send(
                            f"TIC-TAC-TOE!: User {ctx.author.mention} has won!"
                        )
        ...
        os.remove(f"{PROJECT_DIR}/assets/tictactoe/gridNEW.png")
