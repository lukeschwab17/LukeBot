...
class Database:
    def __init__(self):
        self.db_file_location = DATABASE
    ...
    def cmd_to_db(cls, cmd: str, server: str, user: str, context: str = None):
        with cls() as db:
            ...
            db.curs.execute(
                "INSERT INTO command_calls VALUES (?, ?, ?, ?, ?)",
                (
                    cmd,
                    server,
                    user,
    ...
    def store_servers_data(cls, serverData: set):
        with cls() as db:
            db.curs.execute("""DROP TABLE server_users""")
            db.curs.execute(
            ...
            for data in serverData:
                db.curs.execute(
                    "INSERT OR IGNORE INTO server_users VALUES (?, ?, ?, ?, ?)", data
                )
    ...
    @classmethod
    def get_largest_rowid(
        cls, table_name: str
    ) -> (
        int
    ):  # yes its redundant i couldnt figure out how to call getIDS() inside this function
        with cls() as db:
            db.curs.execute(f"SELECT rowid FROM {table_name}")
            all_ids = [row[0] for row in db.curs.fetchall()]
            return max(all_ids)
    ...
    def get_last_item(cls, table_name: str) -> tuple:
        with cls() as db:
            db.curs.execute(
                f"SELECT rowid, * FROM {table_name} ORDER BY rowid DESC LIMIT 1"
            )
    ...
    def check_existing_avatar_url(cls, avatar_url: str) -> bool:
        ...
        with cls() as db:
            db.curs.execute(
                "Select image_url FROM avatars WHERE image_url = ?", (avatar_url,)
            )
    ...
    def store_avatar_data(cls, avatar_url: str, avatar_name: str, submitter_id: str):
        with cls() as db:
            ...
            db.curs.execute(
                "INSERT INTO avatars (image_url, avatar_name, submitter_id) VALUES (?, ?, ?)",
            ...
            db.curs.execute(
                "UPDATE avatars SET file_path = ? WHERE rowid = ?",
                (file_path, db.curs.lastrowid),
            )  # Update the file path for the last inserted row
    ...
    def get_avatar_file_path(cls, avatar_url: str) -> str:
        with cls() as db:
            db.curs.execute(
                "SELECT file_path FROM avatars WHERE image_url = ?", (avatar_url,)
            )
            selected_url = db.curs.fetchone()
    ...
    def submit_video(cls, user_ids: set, numclips: int, clip_name: str, guild_id: str):
        ...
        with cls() as db:
            ...
            db.curs.execute(
                "INSERT into recording_session_compilations (user_ids, number_of_clips, recording_name, guild_id) VALUES (?, ?, ?, ?)",
